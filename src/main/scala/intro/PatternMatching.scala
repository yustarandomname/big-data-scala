...
object PatternMatching {
    ...
    def onlyEvenNumbers(xs: List[OptionalNum]): List[Int] = xs match {
        case Num(x) :: t if (x % 2 == 0) => x :: onlyEvenNumbers(t) // if head is even
        case Num(x) :: t if (x % 1 == 0) => onlyEvenNumbers(t) // if head is odd

        case Nothing() :: t => onlyEvenNumbers(t) // if head is Nothing

        case Nil => Nil // if head is Nil
    }
}
